...
| 字段 | 类型 | 说明 |
| ---- | ---- | ---- |
| safe_code | string | 在 `NeedTfa(11028)` / `NeedResetPWD(11015)` / `NeedBindOAuth(11042)` 返回 |
...
- `oauth_bind` 场景示例：
  ```json
  {
    "action": "oauth_bind",
    "oauth_type": "feishu",
    "oauth_id": "on-d89jhsdhjsajkda7828enjdj328ydhhw3u43yjhdj",
    "oauth_profile": {
      "user_name": "张三",
  ...
  ```
- `action` 可为：`tfa`、`reset_password`、`oauth_bind`
...
### 4.7 当前用户信息结构（`GET /profile` 响应 `data`）
| 字段 | 类型 | 说明 |
| ---- | ---- | ---- |
| id | uint | 用户 ID |
...
3. 按 `feishu_id` / `wechat_id` 查询后台用户并生成 token。
   其中 `feishu_id` 当前存储飞书 `union_id`。
   其中 `wechat_id` 当前存储企业微信 `userid`。
4. 若第三方未绑定用户，返回 `11042` 且 `safe_code(action=oauth_bind)`。
   当前仅支持绑定已存在的后台用户，不支持通过第三方登录自动创建用户。
5. `NeedBindOAuth` 场景会同时返回第三方资料预览与可同步字段列表；前端可让用户勾选本次要同步的字段。

### 5.5 第三方账号绑定确认（`POST /oauth/bind`）
1. 传入 `safe_code`（来自 `11042` 响应）与本地 `identifier`。
2. 解析 `safe_code`，要求 `action=oauth_bind`。
3. 使用 `identifier` 查本地用户。
4. 若用户开启 TFA，校验 `totp_code`；否则校验 `password`。
5. 前端可选传 `sync_fields`，指定本次要同步的字段。
6. 校验目标第三方 ID 未被其他用户绑定。
7. 绑定成功后写入第三方 ID；若 `sync_fields` 非空，则同步对应资料字段。
...
## 6.9 确认绑定第三方
- **Method**：POST
- **Path**：`/go-api/internal/admin/auth/oauth/bind`
- **鉴权**：否
- **Body**：

  | 字段 | 类型 | 必填 | 说明 |
  | ---- | ---- | ---- | ---- |
  | safe_code | string | 是 | `POST /token` 返回的绑定安全码 |
  | identifier | string | 是 | 本地邮箱/手机号 |
  | password | string | 条件必填 | 未开启 TFA 时必填，传 `md5(当前明文密码)` |
  | totp_code | string | 条件必填 | 开启 TFA 时必填 |
...
- **错误码**：`400`、`11034`、`11014`、`11030`、`11007`、`11002`、`11033`、`11029`、`11000`、`11001`、`11043`、`500`
...
5. `POST /oauth/bind`：
   - 未开启 TFA：`password` 必须传 `md5(当前明文密码)`；
   - 开启 TFA：必须传 `totp_code`；
