...
- **Body**：
  ...
  | password | string | 是 | 新密码，前端必须传 `md5(明文密码)`（服务端再按 `MD5(password + salt)` 存储） |
...
- **Body**：
  ...
  | password | string | 是 | 新密码，前端必须传 `md5(新明文密码)`（服务端再按 `MD5(password + salt)` 存储） |
...
当前实现中，密码相关校验统一按 `MD5(请求密码字段 + salt)` 进行，因此前端必须先对明文密码做一次 MD5 后再传参。
