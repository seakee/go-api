...
本文档按当前代码实现整理，覆盖 `app/http/router/internal/admin/auth/auth.go` 中 16 个路由的完整功能、调用链路和数据结构。
...
- `reauth_ticket`
  ...
  - 用途：高风险操作前确认本地账号所有权
- 两类 ticket 都不会在读取时立即消费，只有绑定/解绑成功后才删除。
...
2. 使用密码验证本地账号所有权。
3. 若用户已开启 TFA，再要求并校验 `totp_code`。
4. 成功后返回短期 `reauth_ticket`，仅用于绑定/解绑等高风险操作。
...
- **错误码**：`400`、`11000`、`11001`、`11002`、`11033`、`11029`、`500`
...
5. `POST /reauth`：
   - 始终需要 `password=md5(当前明文密码)`；
   - 若用户已开启 TFA，还必须传 `totp_code`。
