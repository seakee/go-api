...
- `trace.id` comes from the `X-Trace-ID` request header or is generated by the server.
- `trace.desc` is usually an empty string and only includes error details in non-production debug scenarios.
